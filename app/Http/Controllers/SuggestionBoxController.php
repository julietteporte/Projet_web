...
class SuggestionBoxController extends Controller
{
    ...
    {
        $manifestations = Manifestation::withCount([
            'votePour',
            'voteContre'
        ])->get();
    ...
    public function addLike($ID, $TYPE)
    {
        if (Auth::check()) {
            ...
            if (isset($typeVote)) {
                $vote = Vote::where('ID_Manifestation', $ID)->where('ID_TypeVote', $typeVote->ID)
                    ->where('ID_Compte', $iduser)
                    ->first();
                if (isset($vote)) {
                    Vote::where('ID_Manifestation', $ID)->where('ID_TypeVote', $typeVote->ID)
                        ->where('ID_Compte', $iduser)
                        ->delete();
                } else {
                    Vote::where('ID_Manifestation', $ID)->where('ID_Compte', $iduser)->delete();
